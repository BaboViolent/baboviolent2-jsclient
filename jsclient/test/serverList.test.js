...
test('server probes omit offline targets and sort measured ping', async () => {
  const fakeFetch = async (url) => {
    ...
    return {
      ok: true,
      json: async () => ({ name: url.includes('slow') ? 'Slow' : 'Fast', map: 'CTF-Alert', players: 1, maxPlayers: 8 }),
    };
  };
  ...
  assert.deepEqual(servers.map((server) => server.name), ['Fast', 'Slow']);
  assert.equal(servers[0].port, 8081);
  assert.ok(servers[1].ping >= servers[0].ping);
});
