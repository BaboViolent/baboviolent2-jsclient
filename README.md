...
BV2_BIND=127.0.0.1:8081 BV2_CONTENT_ROOT=../Content/main \
  BV2_MAP=CTF-Alert BV2_GAME_TYPE=2 BV2_BOTS=12 BV2_DIAGNOSTICS=1 \
  RUST_LOG=bv2_server=info,bv2_diagnostics=info cargo run --release 2>&1 | tee -a /tmp/bv2-server.log
```
...
server and browser logs now include matching `world-entities` inventories for
rockets, grenades, molotovs, flames, health, dropped weapons, and grenade packs.
